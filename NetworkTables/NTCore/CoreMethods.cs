...
namespace NetworkTables.NTCore
{
    public class CoreMethods
    {
        ...
        private static readonly Dictionary<uint, NT_EntryListenerCallback> s_entryCallbacks =
            new Dictionary<uint, NT_EntryListenerCallback>();

        public static uint AddEntryListener(string prefix, ITable table, ITableListener listener,
            bool immediate_notify)
        {
            NT_EntryListenerCallback modCallback = (uid, ptr, name, len, value, is_new) =>
            {
                ...
                listener.ValueChanged(table, key, obj, is_new != 0);
            };
        ...
        private static readonly Dictionary<uint, NT_ConnectionListenerCallback> s_connectionCallbacks =
            new Dictionary<uint, NT_ConnectionListenerCallback>();

        public static uint AddConnectionListener(CoreConnections.ConnectionListenerCallback callback)
        {
            NT_ConnectionListenerCallback modCallback =
                (uint uid, IntPtr data, int connected, ref NT_ConnectionInfo conn) =>
                {
                    callback(uid, connected != 0, conn);
                };

            uint retVal = NT_AddConnectionListener(IntPtr.Zero, modCallback);
        ...
        {
            NT_RemoveConnectionListener(uid);
